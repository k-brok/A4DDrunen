// app/api/account/tickets/[registrationId]/[participantId]/route.ts
...
export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ registrationId: string; participantId: string }> },
) {
  const { registrationId, participantId } = await params
  ...
  const pdfBytes = await generateRegistrationTicketsPdf(payload, registrationId, { participantId })

  return new NextResponse(Buffer.from(pdfBytes), {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': `inline; filename="ticket-${participantId}.pdf"`,
    },
  })
}
