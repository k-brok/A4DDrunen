...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0 ring-ring/10 dark:ring-ring/20 dark:outline-ring/40 outline-ring/50 focus-visible:ring-4 focus-visible:outline-1 aria-invalid:focus-visible:ring-0",
  {
    variants: {
      ...
      size: {
        clear: '',
        default: 'h-10 px-4 py-2 has-[>svg]:px-3 font-medium text-sm',
        sm: 'h-9 rounded-md px-3 has-[>svg]:px-2.5 font-medium text-sm ',
        lg: 'h-11 rounded-md px-8 has-[>svg]:px-4 font-medium text-sm ',
        xl: 'h-16 rounded-md px-10 has-[>svg]:px-5 font-bold text-xl',
        icon: 'size-10',
      },
    },
    defaultVariants: {
